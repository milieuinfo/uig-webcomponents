 ...
 */
export class VlPagerComponent extends vlElement(HTMLElement) {
  ...
  get _pageSkippedElements() {
    return [
      ...this._pagesListElement.querySelectorAll(
        "[data-vl-pager-page-skipped]"
      ),
    ];
  }
  ...
  _updatePagination() {
    if (this._isPagination) {
      ...
      if (this.totalPages > 1) {
        const pages = this.__generatePagination(
          this.currentPage,
          this.totalPages
        );
        const templates = pages.map((number) => this._getPageTemplate(number));
        templates.forEach((template) =>
          this._pagesListElement.insertBefore(
            template,
            this._pageForwardListItem
          )
        );
      }
    }
