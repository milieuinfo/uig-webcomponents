...
import "../../../../elements/src/lib/icon/vl-icon.element";
...
export class VlAlert extends vlElement(HTMLElement) {
  ...
  connectedCallback() {
    ...
    this._actionsSlotElement.addEventListener("slotchange", () =>
      this.__processButtons()
    );
    this._titleSlotElement.addEventListener("slotchange", () =>
      this.__processTitleElementVisibility()
    );
  }
  ...
  _closableChangedCallback(oldValue: string, newValue: string) {
    ...
    if (newValue != undefined) {
      ...
      closeButtonTemplate
        ...
        .addEventListener("click", this.__removeAlert);
  ...
  __processActionsElementVisibility() {
    this._actionsElement.hidden =
      ...
      this._actionsSlotElement.assignedElements().length == 0;
  }
