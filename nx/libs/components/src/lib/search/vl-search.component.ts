...
import { VlSelect } from "../../../../elements/src/lib/select/vl-select.element";
 ...
 */
export class VlSearchComponent extends vlElement(HTMLElement) {
      ...
      customElements.whenDefined("vl-select").then(async () => {
        if (slot instanceof VlSelect) {
          ...
          this.__observeInputSlot((mutations: any) => {
            const isOpen = (mutation: any) =>
              mutation.target.classList.contains("is-open");
            const isFocused = (mutation: any) =>
              mutation.target.classList.contains("is-focused");
            if (
              mutations.find(
                (mutation: any) => isOpen(mutation) || isFocused(mutation)
              ) ||
              slot.value
            ) {
