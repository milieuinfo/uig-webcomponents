...
export default class VlPager extends VlElement {
  ...
  async getCurrentPage() {
    const label = await this.shadowRoot.findElement(
      By.css("[data-vl-pager-page] label")
    );
  ...
  async goToLastPage() {
    const number = Math.ceil(
      (await this.getTotalItems()) / (await this.getItemsPerPage())
    );
  ...
  async _getPageLink(number) {
    return this.shadowRoot.findElement(
      By.css(`[data-vl-pager-page="${number}"] a`)
    );
  }
