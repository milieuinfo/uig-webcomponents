 ...
 * @mixes vlElement
 */
export class VlSearchResult extends vlElement(HTMLLIElement) {
  get _classPrefix() {
  ...
  _processMetaData() {
    if (this._metaDataElement) {
      this._sectionElement.insertAdjacentHTML(
        "beforeend",
        this._metaDataContainerTemplate
      );
  ...
  _processContent() {
    this._contentElements.forEach((element: Element) => {
      const container = this._template(
        this._contentContainerTemplate
      ).firstElementChild;
  ...
  _setContentClasses(element: Element) {
    ...
    element
      .querySelectorAll("dt")
      .forEach((dt) => dt.classList.add(`${dlClass}__description`));
  }
}
