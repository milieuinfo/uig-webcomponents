...
export class VlInfotextElement extends vlElement(HTMLDivElement) {
  ...
  connectedCallback() {
    if (this.__hasOneChild()) {
      ...
      this._childObserver = this.__observeChildElement((records: any) => this.__processChildElementChange(records));
