...
export class VlMultiSelect extends VlSelect {
  ...
  async _getUnselectedItems() {
    ...
    return Promise.all(items.map(async (item) => new MultiselectItem(item)));
  }

  async _getSelectedItems() {
    const root = await this._getRoot();
    const selectedItems = await root.findElements(By.css('.vl-pill'));
    return Promise.all(selectedItems.map(async (item) => new Pill(item)));
  }
