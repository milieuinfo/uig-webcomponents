...
export class VlSearchFilter extends nativeVlElement(HTMLDivElement) {
  ...
  __processClasses() {
    this.querySelectorAll("form").forEach((form) =>
      form.classList.add(`${this._elementPrefix}form`)
    );
    this.querySelectorAll("form > section").forEach((section) =>
      section.classList.add(`${this._elementPrefix}section`)
    );
    this.querySelectorAll("form > section > h2").forEach((title) =>
      title.classList.add(`${this._elementPrefix}section-title`)
    );
    this.querySelectorAll("form > section > div").forEach((field) =>
      field.classList.add(`${this._elementPrefix}field`)
    );
    this.querySelectorAll("form > section > div > label").forEach((label) =>
      label.classList.add(`${this._elementPrefix}field__label`)
    );
    this.querySelectorAll("form > div").forEach((div) =>
      div.classList.add(`${this._elementPrefix}field`)
    );
  ...
  get _mobileModalTitleElement() {
    return this._headerElement
      ? this._headerElement.querySelector("h2")
      : undefined;
  }
  ...
  get _footerElement() {
    return this.querySelector(
      `form ~ div:not(.${this._elementPrefix}footer-modal):last-child`
    );
  }
  ...
  get _submitButton() {
    ...
    if (this._formElement) {
      button = this._formElement.querySelector(
        ':scope > div button[type="submit"]'
      );
    }
  ...
  get _mobileModalTitle() {
    return (
      this.getAttribute("data-vl-mobile-modal-title") || this._title || "Filter"
    );
  }
  ...
  __prepareMobileModal(element) {
    ...
    [...element.children].forEach((child) =>
      child.classList.toggle(`${this._elementPrefix}header__clear`)
    );
  }
