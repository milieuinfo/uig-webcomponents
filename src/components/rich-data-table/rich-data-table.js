...
export class VlRichDataTable extends VlRichData {
  ...
  get __richDataFields() {
    return [...this.__fields].filter(
      (field) => field.constructor === VlRichDataField
    );
  }
  ...
  _renderHeaders() {
    ...
    const headerColumns = this.__richDataFields.map((field) =>
      field.headerTemplate()
    );
    const atLeastOneHeaderColumnHasContent = headerColumns.some(
  ...
  __fieldChanged(event) {
    ...
    if (propertiesChanged) {
      if (
        propertiesChanged.some((property) =>
          VlRichDataField.headerAttributes.includes(property)
        )
      ) {
        this._renderHeaders();
      }
      ...
      if (
        propertiesChanged.some((property) =>
          VlRichDataField.bodyAttributes.includes(property)
        )
      ) {
        this._renderBody();
      }
  ...
  __sortingChanged(event) {
    if (this._isMultisortingEnabled) {
      this.__activeSorters.forEach(
        (sorter, index) => (sorter.priority = index + 1)
      );
    } else {
...
Promise.all([
  customElements.whenDefined(VlRichDataField.is),
  customElements.whenDefined(VlRichDataSorter.is),
