...
import VlDataTable from "../../../data-table/test/e2e/data-table";
...
class VlRichDataTable extends VlRichData {
  ...
  async getSorter(field) {
    const element = await this.shadowRoot.findElement(
      By.css(`vl-rich-data-sorter[data-vl-for="${field}"]`)
    );
  ...
  async _getTable() {
    const element = await this.shadowRoot.findElement(
      By.css('[is="vl-data-table"]')
    );
    return new VlDataTable(this.driver, element);
  }
}

export default VlRichDataTable;
