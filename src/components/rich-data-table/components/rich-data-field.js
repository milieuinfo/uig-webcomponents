...
export class VlRichDataField extends vlElement(HTMLElement) {
  ...
  get __headerContent() {
    return (
      this.label ||
      (this._labelSlotElement ? this._labelSlotElement.innerHTML : undefined)
    );
  }
    ...
    if (content) {
      if (this.sortable) {
        const direction = this.sortingDirection
          ? `data-vl-direction="${this.sortingDirection}"`
          : "";
        const priority = this.sortingPriority
          ? `data-vl-priority="${this.sortingPriority}"`
          : "";
  ...
  __getValueContentElement(data) {
    if (this.selector) {
      ...
      const template = ((literal, item) =>
        new Function("item", "return `" + literal + "`")(item)).call(
          this,
          literal,
          data
        );
