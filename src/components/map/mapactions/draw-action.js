...
export class VlDrawAction extends VlMapAction {
  constructor(layer, type, onDraw, options = {}) {
          ...
          if (options.snapping.layer) {
            interactions.push(
              new VlSnapInteraction(
                options.snapping.layer.getSource(),
                options.snapping
              )
            );
          } else {
    ...
    drawInteraction.on("drawstart", (event) => {
      if (options.measure) {
        ...
        options.measure =
          typeof options.measure === "object" ? options.measure : {};
  ...
  _getLengthOfLastSegment(geometry) {
    if (geometry && geometry instanceof LineString) {
      return new LineString(this._getCoordinatesOfLastSegment(geometry))
        .getLength()
        .toFixed(2);
    } else if (geometry && geometry instanceof Polygon) {
      return new LineString(
        this._getCoordinatesOfLastSegment(geometry.getLinearRing(0))
      )
        .getLength()
        .toFixed(2);
    }
