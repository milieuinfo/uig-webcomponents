...
export class VlSelectActions extends VlSelectAction {
  constructor(layerConfiguraties, onSelect, options) {
    const layers = layerConfiguraties.map(
      (layerConfiguratie) => layerConfiguratie.layer
    );
  ...
  _getLayer(feature) {
    return this.layers.find(
      (layer) => layer.getSource().getFeatures().indexOf(feature) !== -1
    );
  }

  _getLayerStyle(layer, type) {
    const layerConfiguratie = this.layerConfiguraties.find(
      (layerConfiguratie) => layerConfiguratie.layer === layer
    );
  ...
  _getLayerHoverStyle(layer) {
    return (
      this._getLayerStyle(layer, "hoverStyle") || this._getLayerStyle(layer)
    );
  }
