...
import { VlMapFeaturesLayerPage } from "../pages/vl-map-features-layer.page";
...
describe("vl-map-features-layer", async () => {
  ...
  it("als gebruiker zie ik het verschil tussen een clustered layer en niet geclusterde layer", async () => {
    const standardLayer = (
      await (await page.getMapWithStandardLayer()).getLayers()
    )[0];
    const clusteredLayer = (
      await (await page.getMapWithClusteredLayer()).getLayers()
    )[0];
  ...
  it("als gebruiker zie ik het verschil tussen een auto extent layer en niet auto extent layer", async () => {
    ...
    const autoExtentLayer = (
      await (await page.getMapWithAutoExtentLayer()).getLayers()
    )[0];
