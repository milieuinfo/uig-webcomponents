...
describe('composite wfs source', () => {
  ...
  it('queries all vector sources and adds all features', (done) => {
    ...
    const source1 = new OlVectorSource({
      ...
      url: (extent, resolution, projection) => {
        expect(extent).to.equal(extent);
        expect(resolution).to.equal(resolution);
        expect(projection).to.equal(projection);
   ...
   `;
    ...
    setTimeout(() => {
      expect(compositeVectorSource.getFeatures().length).to.equal(2);
      expect(compositeVectorSource.getFeatures()[0].getGeometry() instanceof MultiPolygon).to.be.true;
      expect(compositeVectorSource.getFeatures()[0].getGeometry().getPolygons()[0].getCoordinates()).to.deep.equal([
        [
          [0, 0, 0],
          [0, 10, 0],
          [10, 10, NaN],
      ...
      expect(compositeVectorSource.getFeatures()[1].getGeometry() instanceof MultiLineString).to.be.true;
      expect(compositeVectorSource.getFeatures()[1].getGeometry().getLineStrings()[0].getCoordinates()).to.deep.equal([
        [0, 0, 0],
        [20, 20, 0],
      ]);
      done();
