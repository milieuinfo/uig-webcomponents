...
import VlGridPage from "./grid.page";
...
describe("vl-grid", async () => {
  ...
  it("men kan een grid links aligneren", async () => {
    ...
    await assert.eventually.isTrue(
      startAlignedGrid.isHorizontallyStartAligned()
    );
  });
  ...
  it("men kan een grid in het midden aligneren", async () => {
    ...
    await assert.eventually.isTrue(
      centerAlignedGrid.isHorizontallyCenterAligned()
    );
  });
  ...
  it("men kan zoveel mogelijk ruimte rond kolommen laten", async () => {
    const alignedWithSpaceAroundGrid =
      await vlGridPage.getAlignedWithSpaceAroundGrid();
    await assert.eventually.isTrue(
      alignedWithSpaceAroundGrid.isHorizontallyAlignedWithSpaceAround()
    );
  });

  it("men kan zoveel mogelijk ruimte tussen kolommen laten", async () => {
    const alignedWithSpaceBetweenGrid =
      await vlGridPage.getAlignedWithSpaceBetweenGrid();
    await assert.eventually.isTrue(
      alignedWithSpaceBetweenGrid.isHorizontallyAlignedWithSpaceBetween()
    );
  });

  it("men kan verticaal aligneren aan de top", async () => {
    const verticallyTopAlignedGrid =
      await vlGridPage.getVerticallyTopAlignedGrid();
    await assert.eventually.isTrue(
      verticallyTopAlignedGrid.isVerticallyTopAligned()
    );
  });
  ...
  it("men kan verticaal in het midden aligneren", async () => {
    const verticallyCenterAlignedGrid =
      await vlGridPage.getVerticallyCenterAlignedGrid();
    await assert.eventually.isTrue(
      verticallyCenterAlignedGrid.isVerticallyCenterAligned()
    );
  });

  it("men kan onderaan aligneren", async () => {
    const verticallyBottomAlignedGrid =
      await vlGridPage.getVerticallyBottomAlignedGrid();
    await assert.eventually.isTrue(
      verticallyBottomAlignedGrid.isVerticallyBottomAligned()
    );
  });

  it("men kan kolommen tot aan hun maximale hoogte rekken", async () => {
    const verticallyStretchedGrid =
      await vlGridPage.getVerticallyStretchedGrid();
    await assert.eventually.isTrue(
      verticallyStretchedGrid.isVerticallyStretched()
    );
  });
});
...
describe("vl-column", async () => {
  ...
  it("men kan er op vertrouwen dat er default sizes worden gekozen", async () => {
    ...
    await assert.eventually.equal(
      defaultMaxSizeColumn.getExtraSmallMaxSize(),
      12
    );
  });
