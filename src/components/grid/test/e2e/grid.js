...
class VlColumn extends VlElement {
  ...
  async _getSize(responsiveModifier) {
    const sizeMatcher = new RegExp(
      `^${this._columnClassPrefix
      }(\\d+)-(\\d+)${this.__responsiveModifierClassPostfix(
        responsiveModifier
      )}\$`
    );
  ...
  async _parseMatchingClass(classMatcher) {
    const clazz = await this._getFirstMatchingClass(
      classMatcher.test.bind(classMatcher)
    );
...
export { VlRegion, VlLayout, VlGrid, VlColumn };
