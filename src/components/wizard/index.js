...
export class VlWizard extends vlElement(HTMLElement) {
  ...
  _processPanes() {
    this._panes.forEach((pane) => {
      this._progressBar.appendChild(
        this._getProgressBarStepTemplate(pane.title)
      );
    });
  ...
  _observeProgressBarClick() {
    setTimeout(() => {
      this._progressBar.buttons.forEach(
        (button) =>
          (button.onclick = (event) => {
            const number = Number(event.target.getAttribute("data-vl-index"));
            if (number < this._activePaneNumber) {
              const panesBetween = this.panes.slice(
                number,
                this.panes.indexOf(this._activePane) + 1
              );
              const allPanesBetweenAreEnabled = panesBetween.every(
                (pane) => !pane.isPreviousPaneDisabled
              );
              this.callback = allPanesBetweenAreEnabled
                ? Promise.resolve(true)
                : new Promise(() => {});
            } else {
              const panesBetween = this.panes.slice(
                this.panes.indexOf(this._activePane),
                number - 1
              );
              const allPanesBetweenAreEnabled = panesBetween.every(
                (pane) => !pane.isNextPaneDisabled
              );
              this.callback = allPanesBetweenAreEnabled
                ? Promise.resolve(true)
                : new Promise(() => {});
    ...
    setTimeout(() => {
      this._progressBar.onmouseover = () => {
        this._progressBar.buttons.forEach((button, index) => {
          ...
          const paneIsDisabled = previousPane
            ? previousPane.isNextPaneDisabled
            : nextPane.isPreviousPaneDisabled;
          if (paneIsDisabled) {
  ...
  _observePanes(callback) {
    const observer = new MutationObserver((mutations) => {
      const hasNewPane = mutations
        .flatMap((mutation) => [...mutation.addedNodes])
        .some((node) => node instanceof VlWizardPane);
      if (hasNewPane) {
        this.constructor._observedAttributes.forEach((attribute) =>
          this._triggerAttribute(attribute)
        );
        callback();
      }
    });
    observer.observe(this._panesSlot, {
      childList: true,
      subtree: true,
