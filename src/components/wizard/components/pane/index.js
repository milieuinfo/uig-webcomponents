...
export class VlWizardPane extends vlElement(HTMLElement) {
  ...
  connectedCallback() {
    ...
    this._activeObserver = this._observeActiveClass(() =>
      this._dispatchActiveEvent()
    );
  }
  ...
  get _previousAction() {
    return this._getAssignedElementByIndex(
      this._previousActionSlotPlaceholder,
      0
    );
  }
  ...
  _observeActionsClick() {
    ...
    if (previousAction) {
      this._previousAction.addEventListener("click", () => {
        this.isPreviousPaneDisabled
          ? this.disablePreviousPane()
          : this.enablePreviousPane();
      });
    }
    if (nextAction) {
      this._nextAction.addEventListener("click", () => {
        this.isNextPaneDisabled
          ? this.disableNextPane()
          : this.enableNextPane();
      });
    }
  }
  ...
  _observeActiveClass(callback) {
    const observer = new MutationObserver((mutations) => {
      if (this.isActive) {
        if (
          !this.__wasActive &&
          mutations.some((mutation) => mutation.target.isActive)
        ) {
          callback();
        }
