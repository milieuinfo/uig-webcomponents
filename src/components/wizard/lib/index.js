(function (global, factory) {
          ...
          value: function updateStep(progressbar, stepIndex) {
            ...
            vl.util.each(steps, function (step) {
              var currentStep = steps[stepIndex - 1],
                ...
                currentStepBullet = currentStep.querySelector(
                  ".".concat(pbStepBulletClass)
                );
  ...
  var _setPaneVisibility = function _setPaneVisibility(
    panes,
    navigationIsForward
  ) {
    vl.util.each(panes, function (pane) {
      ...
      pane.setAttribute(
        "aria-hidden",
        vl.util.hasClass(pane, "is-selected") ? "false" : "true"
      );
  ...
  var _setTabbing = function _setTabbing(panes) {
    ...
    vl.util.each(panes, function (pane) {
      var paneFocusItems = pane.querySelectorAll(
        "[".concat(
          wizfocusAtt,
          "], input, textarea, select, button, a, area, iframe"
        )
      );
  ...
  var _createWizard = function _createWizard(
    wizard,
    params,
    paneElements,
    stepElements,
    ...
    var panes =
      paneElements || wizard.querySelectorAll(".".concat(wizPaneClass));
    var nPanes = panes.length; // 1e element is "0"

    var ctasNext = [...panes]
      .map((pane) => pane.querySelector("[" + wizNextAtt + "]"))
      .filter(Boolean);
    var ctasPrev = [...panes]
      .map((pane) => pane.querySelector("[" + wizPrevAtt + "]"))
      .filter(Boolean);
    var pbSteps =
