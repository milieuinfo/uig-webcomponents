...
export class VlDatepicker extends vlFormValidationElement(
  ...
  dress() {
    if (!this._dressed) {
      awaitUntil(() =>
        this._inputElement.classList.contains("vl-input-field")
      ).then(() => {
    ...
    if (oldValue) {
      ...
      switch (newValue) {
        case "time":
          ...
          this._element.setAttribute(
            this._attributePrefix + "disable-date",
            "true"
          );
          break;
        case "date-time":
  ...
  _visualFormatChangedCallback(oldValue, newValue) {
    this._element.setAttribute(
      this._attributePrefix + "visual-format",
      newValue
    );
  }
  ...
  _selectedDateChangedCallback(oldValue, newValue) {
    this._element.setAttribute(
      this._attributePrefix + "selected-date",
      newValue
    );
  }
  ...
  _amPmChangedCallback(oldValue, newValue) {
    this._element.setAttribute(
      this._attributePrefix + "24hr-time",
      newValue == undefined
    );
  }
