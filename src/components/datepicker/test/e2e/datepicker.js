...
class VlDatepicker extends VlElement {
  ...
  async getVisualisedValue() {
    const input = await this.shadowRoot.findElement(
      By.css(".js-vl-datepicker-input:not(#input)")
    );
    return input.getAttribute("value");
  }
  ...
  async selectMinutes(minutes) {
    await this.open();
    const input = await new VlElement(
      this.driver,
      await this._getMinuteInput()
    );
  ...
  async _getMonthSelect() {
    const select = await this.shadowRoot.findElement(
      By.css("select.flatpickr-monthDropdown-months")
    );
  ...
  async _getDayElements() {
    return this.shadowRoot.findElements(
      By.css("span.flatpickr-day:not(.prevMonthDay):not(.nextMonthDay)")
    );
  }
  ...
  async _getInputElement() {
    return new VlInputField(
      this.driver,
      await this.shadowRoot.findElement(By.id("input"))
    );
  }
...
class VlMonthSelect extends VlElement {
  ...
  async select(month) {
    ...
    const option = (await this._mapVisibleText(options)).find(
      (m) => m.visibleText === month
    );
