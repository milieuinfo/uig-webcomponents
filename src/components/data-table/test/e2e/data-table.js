import { VlElement, By, assert } from '../../../../utils/test';

export class VlDataTable extends VlElement {
  async getCaption() {
...
export class VlDataTableRow extends VlElement {
  async getCells() {
...
export class VlDataTableCell extends VlElement {
  async getRowSpan() {
  ...
  async getButtons() {
    ...
    return Promise.all(buttons.map((button) => new VlDataTableCellButton(this.driver, button)));
  }

  async getElement(by) {
    const elements = await this.findElements(by);
    return Promise.all(elements.map((element) => new VlElement(this.driver, element)));
  }
}

export class VlDataTableCellButton extends VlElement {}
