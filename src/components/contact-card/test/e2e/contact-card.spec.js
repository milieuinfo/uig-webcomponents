...
describe('vl-contact-card', async () => {
  ...
  it('als gebruiker kan ik de contact card info bekijken', async () => {
    ...
    await assert.eventually.equal(title[0].getText(), 'Departement Onderwijs en Vorming');
  });
  ...
  it('als gebruiker kan ik de contact card properties bekijken', async () => {
    ...
    await assert.eventually.include(adresProperty.value.getText(), 'Hendrik Consciencegebouw');
    await assert.eventually.include(adresProperty.value.getText(), 'Koning Albert II-laan 15');
    await assert.eventually.include(adresProperty.value.getText(), '1210 Brussel');
    await assert.eventually.include(adresProperty.value.getText(), 'Routeplanner');
    const phoneProperty = properties[1];
    await assert.eventually.equal(phoneProperty.term.getText(), 'Telefoon');
    await assert.eventually.include(phoneProperty.value.getText(), '02 553 72 02');
    await assert.eventually.include(phoneProperty.value.getText(), '(Onthaal Consciencegebouw)');
    await assert.eventually.include(phoneProperty.value.getText(), '1700');
    await assert.eventually.include(phoneProperty.value.getText(), '(Infolijn Onderwijs)');
    const mailProperty = properties[2];
    await assert.eventually.equal(mailProperty.term.getText(), 'E-mail');
    await assert.eventually.include(mailProperty.value.getText(), 'onderwijs.vlaanderen@vlaanderen.be');
    const websiteProperty = properties[3];
    await assert.eventually.equal(websiteProperty.term.getText(), 'Website');
    await assert.eventually.include(websiteProperty.value.getText(), 'http://onderwijs.vlaanderen.be');
  });
});
