...
import { VlSelectOptionGroup } from "./vl-select-option-group";

export class VlSelect extends VlElement {
  async open() {
  ...
  async groups() {
    if (await this.isDressed()) {
      ...
      const elements = await Promise.all(
        (
          await selectList.findElements(
            By.css(".vl-select__group, .vl-select__item")
          )
        ).map((element) => new VlElement(this.driver, element))
      );
      ...
      return selectGroups.map((group, index) => {
        ...
        const to =
          index + 1 === selectGroups.length
            ? elements.length
            : elements.indexOf(selectGroups[index + 1]);
      ...
      const optGroupElementPromises = optGroupElements.map(
        async (optGroupElement) => {
  ...
  async _getOptions() {
    if (await this.isDressed()) {
      const selectList = await this._getSelectList();
      const selectItems = await selectList.findElements(
        By.css(".vl-select__item")
      );
  ...
  async selectByValue(value) {
    if (await !this.hasValue(value)) {
      return Promise.reject(
        new Error("Value " + value + " niet gevonden in select!")
      );
    }
    const options = await this._getOptions();
    const values = await this.values();
  ...
  async selectByText(text) {
    if (await !this.hasText(text)) {
      return Promise.reject(
        new Error("Text " + text + " niet gevonden in select!")
      );
    }
    const options = await this._getOptions();
    const labels = await this.texts();
    return this._clickOption(
      options[labels.findIndex((label) => label === text)]
    );
  }
  ...
  async selectByIndex(index) {
    ...
    if (values.length < index - 1) {
      return Promise.reject(
        new Error("Er zijn maar " + values.length + " values in de dropdown")
      );
    }
    const options = await this._getOptions();
  ...
  async search(text) {
    if (this.hasValue(text) === false) {
      return Promise.reject(
        new Error("Waarde " + text + " niet gevonden in de dropdown!")
      );
    }
    await this.open();
  ...
  async _getSelectList() {
    return (await this._getDressedContainer()).findElement(
      By.css(".vl-select__list")
    );
  }
