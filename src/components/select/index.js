 ...
 */
export class VlSelect extends vlFormValidationElement(
  nativeVlElement(HTMLSelectElement)
) {
  /**
    ...
    if (newValue != null) {
      (async () => {
        if (this._hasDressedAttribute || this._dressed) {
          ...
          this._wrapperElement.parentNode.classList.add(
            "vl-input-field--" + type
          );
        } else {
      ...
      if (this._hasDressedAttribute || this._dressed) {
        this._wrapperElement.parentNode.classList.remove(
          "vl-input-field--" + type
        );
      } else {
  ...
  _setTranslations() {
    this.__searchPlaceholderTranslation =
      this.__searchPlaceholder || this.DEFAULT_SEARCH_PLACEHOLDER;
    this.__searchNoResultTranslation =
      this.__searchNoResults || this.DEFAULT_SEARCH_NO_RESULT;
    this.__noMoreOptionsTranslation =
      this.__noMoreOptions || this.DEFAULT_NO_MORE_OPTIONS;
  }
  ...
  undress() {
    if (this._dressed) {
      try {
        ...
        vl.select.selectInstances.splice(
          vl.select.selectInstances.indexOf(this._choices)
        );
      } catch (exception) {
        console.error(
