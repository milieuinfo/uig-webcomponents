...
export class VlRichData extends vlElement(HTMLElement) {
  ...
  get __filterToggleButtonTextSlot() {
    return this.shadowRoot.querySelector(
      'slot[name="toggle-filter-button-text"]'
    );
  }

  get __filterCloseButtonTextSlot() {
    return this.shadowRoot.querySelector(
      'slot[name="close-filter-button-text"]'
    );
  }
  ...
  set _paging(paging) {
    if (paging) {
      ...
      if (paging.itemsPerPage != null) {
        this.__pager.setAttribute(
          "data-vl-items-per-page",
          paging.itemsPerPage
        );
      }
  ...
  __observePager() {
    if (this.__pager) {
      ...
      this.__pager.addEventListener("change", (e) => {
        ...
        if (
          this.__contentSlot.assignedNodes()[0].children[0].querySelector("a")
        ) {
          this.__contentSlot
            .assignedNodes()[0]
    ...
    if (number) {
      ...
      if (this.__pager) {
        customElements.whenDefined("vl-pager").then(() => {
          this.__numberOfSearchResults.textContent =
            this.__pager.totalItems || 0;
        });
      }
    }
  }
  ...
  __observeMobileModal(callback) {
    ...
    observer.observe(this.__searchFilter, {
      attributeFilter: ["data-vl-mobile-modal"],
    });
    return observer;
  }
