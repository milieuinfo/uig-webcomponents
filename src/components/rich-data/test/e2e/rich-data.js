...
import VlPager from "../../../pager/test/e2e/pager";
import { VlSelect } from "../../../select/test/e2e/select";
import VlSearchFilter from "../../../search-filter/test/e2e/search-filter.js";
...
class VlRichData extends VlElement {
  ...
  async getSearchFilter() {
    const searchFilter = await this.findElement(
      By.css('[is="vl-search-filter"]')
    );
  ...
  async toggleSearchFilter() {
    const button = await this.shadowRoot.findElement(
      By.css("#toggle-filter-button")
    );
    return button.click();
  }
