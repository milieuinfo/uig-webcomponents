...
export const consent = (reference) => {
  ...
  return html`<div is="vl-grid" slot="content" data-vl-is-stacked-small>
    ${fwColumn(html`Departement Omgeving maakt op de websites waarvoor zij verantwoordelijk is gebruik van "cookies" en
    vergelijkbare internettechnieken. Cookies zijn kleine "tekstbestanden" die worden gebruikt om onze websites en apps
    beter te laten werken en jouw surfervaring te verbeteren. Zij kunnen worden opgeslagen in de context van de
    ...
    ${reference.extraOptIns.length === 0
      ? html`${fwColumn(html`Deze website gebruikt enkel noodzakelijke cookies die nodig zijn om de functionaliteit van
        de website mogelijk te maken.`)}
